	...
	<div id="header-container" class="clearfix">
		...
		if ( $sections !== '' ) {
			foreach ($sections as $section => $areas) { ?>
				<div class="header-bulkapparel__section wtbx_hs wtbx_hs_<?php echo esc_attr($section); ?>">
					<div class="wtbx_hs_inner header-bulkapparel__holder">
							<?php
							if ( $section === 'header' ) {
								include(locate_template('templates/header/parts/logo.php'));
							}
						foreach ($areas as $area => $items) { ?>
							<div class="header-bulkapparel__navigation wtbx_ha wtbx_ha_<?php echo esc_attr($section); ?>_<?php echo esc_attr($area); ?> clearfix"><?php
